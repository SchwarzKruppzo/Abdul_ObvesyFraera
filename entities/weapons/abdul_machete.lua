	...
	if tr.Hit then
		if tr.Entity:IsPlayer() or tr.Entity:IsNPC() then
			if tr.MatType == MAT_FLESH or tr.MatType == MAT_ANTLION or tr.MatType == MAT_ALIENFLESH or tr.MatType == MAT_BLOODYFLESH then
				self:EmitSound(Sound("Impact_Flesh.Machete"), 70, 100)
			...
			self:EmitSound(Sound("Impact_World.Machete"), 70, 105)
		end
		self:SendWeaponAnim( ACT_VM_HITCENTER )

	else
		self:EmitSound(Sound("Miss.Machete"), 80, math.random(95, 105))
