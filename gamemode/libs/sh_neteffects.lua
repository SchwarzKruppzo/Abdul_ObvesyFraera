if SERVER then
	...
	util.AddNetworkString("NetParticleEffect")
	util.AddNetworkString("NetParticleEffectAttach")
	
	function netParticleEffect( name, pos, ang, entity )
		net.Start("NetParticleEffect")
	...
	function netParticleEffectAttach( name, attachType, entity, attachID )
		net.Start("NetParticleEffectAttach")
			net.WriteString( name )
			net.WriteString( tonumber( attachType ) )
			net.WriteEntity( entity )
			net.WriteString( tonumber( attachID ) )
		net.Broadcast()
	end
	...
	net.Receive("NetParticleEffect", function( len )
		local name = net.ReadString()
		local ang = net.ReadAngle()
		local pos = net.ReadVector()
		local entity = net.ReadEntity()
		ParticleEffect( name, pos, ang, entity )
	end)
	net.Receive("NetParticleEffectAttach", function( len )
		local name = net.ReadString()
		local attachType = net.ReadString()
		local entity = net.ReadEntity()
		local attachID = net.ReadString()
		
		ParticleEffectAttach( name, tonumber( attachType ), entity, tonumber( attachID ) )
	end)
end